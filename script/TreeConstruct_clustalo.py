...
method = 'sequence_clustalo' # deepblast, TMalign, deepalign
cluster = ''
...
for t in swiss_trees:
    ...
    os.system(f'mkdir -p {folder}/clustalo')
    if len(glob.glob(f'{folder}/structs/*pdb'))<3:
        continue
    if not os.path.exists(folder+'/clustalo/aln.fa'):
        os.system(f'clustalo -i {folder}/seq.fasta -o {folder}/clustalo/aln.fa -v')
    if not os.path.exists(folder+'/clustalo/clustalo_tree.nhx'):
        os.system(f'FastTree {folder}/clustalo/aln.fa > {folder}/clustalo/clustalo_tree.nhx')
    swisst = pickle.load(open(folder+'/swiss_toytree.pkl','rb'))
    out_tree_kernel = folder + '/clustalo/clustalo_tree.nhx'
