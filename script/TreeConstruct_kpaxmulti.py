...
method = sys.argv[1] # kpax_multi or kpax_multi_flex
cluster = ''
...
for t in swiss_trees:
    ...
    os.chdir(folder)
    os.system(f'mkdir -p kpax_multi')
    os.system(f'mkdir -p kpax_multi_flex')
    pdb_list = glob.glob('./structs/*pdb')
    if len(pdb_list)<3:
        continue
    pdbfolder = os.path.basename(pdb_list[0]).replace('.pdb','')
    pdb_list = ' '.join(pdb_list)
    
    if method == 'kpax_multi':
        if not os.path.exists(folder+'/kpax_multi/aln.fa'):
            os.system(f'kpax -multi -nopivot {pdb_list}')
            os.system(f"cp kpax_results/{pdbfolder}/{pdbfolder}_multi.fasta kpax_multi/aln.fa")
        if not os.path.exists(folder+'/kpax_multi/kpax_tree.nhx'):
            os.system(f'FastTree kpax_multi/aln.fa > kpax_multi/kpax_tree.nhx')
    elif method == 'kpax_multi_flex':
        if not os.path.exists(folder+'/kpax_multi_flex/aln.fa'):
            os.system(f'kpax -flex -multi -nopivot {pdb_list}')
            os.system(f"cp kpax_results/{pdbfolder}/{pdbfolder}_multi_flex.fasta kpax_multi_flex/aln.fa")
        if not os.path.exists(folder+'/kpax_multi_flex/kpax_tree.nhx'):
            os.system(f'FastTree kpax_multi_flex/aln.fa > kpax_multi_flex/kpax_tree.nhx')
        pass
    else:
        raise ValueError('Method not accepted!')
    
    swisst = pickle.load(open(folder+'/swiss_toytree.pkl','rb'))
    out_tree_kernel = f'{folder}/{method}/kpax_tree.nhx'
    os.system(f"sed -i 's/_flex//g' {out_tree_kernel}")
